...
import dev.isksss.java.sqlformatter.core.engine.core.util.JSLikeList;
import dev.isksss.java.sqlformatter.core.engine.core.util.Util;
import dev.isksss.java.sqlformatter.core.engine.languages.DialectConfigurator;
...
public abstract class AbstractFormatter implements DialectConfigurator {
  ...
  private String getFormattedQueryFromTokens() {
    ...
    for (Token token : this.tokens) {
      ...
      if (token.type == TokenTypes.LINE_COMMENT) {
        ...
        this.previousReservedToken = token;
      } else if (token.type == TokenTypes.DATA_TYPE) {
        formattedQuery = this.formatWithSpaces(token, formattedQuery);
  ...
  private String formatNewlineReservedWord(Token token, String query) {
    ...
    if (this.cfg.logicalOperatorNewline.equals("after")
        && (Token.isAnd(token) || Token.isOr(token) || Token.isXor(token))) {
      return this.addNewline(this.formatWithSpaces(token, query));
    }
  ...
  private String formatOperator(Token token, String query) {
  ...
  private String formatQuerySeparator(Token token, String query) {
    ...
    if (this.cfg.newlineBeforeSemicolon) {
      return Util.trimSpacesEnd(this.addNewline(query))
          + this.show(token)
  ...
  // Converts token to string according to case options.
  private String show(Token token) {
    if (token.type == TokenTypes.DATA_TYPE) {
      return applyCase(token.value, this.cfg.dataTypeCase);
    }
    if (token.type == TokenTypes.WORD) {
      if (isFunctionName()) {
        return applyCase(token.value, this.cfg.functionCase);
      }
      return applyCase(token.value, this.cfg.identifierCase);
    }
    if (token.type == TokenTypes.RESERVED
      ...
      return applyCase(token.value, this.cfg.keywordCase);
    }
    return token.value;
  }

  private boolean isFunctionName() {
    return Optional.ofNullable(this.tokenLookAhead()).map(token -> token.type == TokenTypes.OPEN_PAREN).orElse(false);
  }

  private String applyCase(String value, String casing) {
